'''
给出R行C列的矩阵，其中的单元格的整数坐标为(r, c)，满足0<=r<R且0<=c<C。另外，我们在该
矩阵中给出了一个坐标为(r0, c0)的单元格。返回矩阵中的所有单元格的坐标，并按到(r0, c0)
...
解释：(r0, c0)到其他单元格的距离为：[0,1,1,2] [[0,1],[1,1],[0,0],[1,0]]也正确答案。
示例 3：输入：R=2, C=3, r0=1, c0 = 2 输出：[[1,2],[0,2],[1,1],[0,1],[1,0],[0,0]]
解释：从 (r0, c0) 到其他单元格的距离为：[0,1,1,2,2,3]
其他满足题目要求的答案也会被视为正确，例如 [[1,2],[1,1],[0,2],[1,0],[0,1],[0,0]]。
提示： 1 <= R <= 100 1 <= C <= 100  0 <= r0 < R  0 <= c0 < C
'''

def allCellsDistOrder(rows, cols, rCenter, cCenter):
    dis = lambda x, y: abs(x - rCenter) + abs(y - cCenter)
    dis_dict = {}
    for i in range(rows):
        for j in range(cols):
            d = dis(i, j)
            if d not in dis_dict:
                dis_dict[d] = [[i, j]]
            else:
                dis_dict[d].append([i, j])
    ans = []
    for d in sorted(dis_dict.keys()):
        ans.extend(dis_dict[d])
    return ans
