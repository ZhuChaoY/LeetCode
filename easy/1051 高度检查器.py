'''
学校在拍年度纪念照时，一般要求学生按照 非递减 的高度顺序排列。请你返回能让所有学生以非
递减高度排列的最小必要移动人数。注意，当一组学生被选中时，他们之间可以以任何可能的方式
重新排序，而未被选中的学生应该保持不动。
...
提示： 1 <= heights.length <= 100   1 <= heights[i] <= 100
'''

def heightChecker(heights):
    _heights = sorted(heights)
    ans = 0
    for x, y in zip(heights, _heights):
        if x != y:
            ans += 1
    return ans
