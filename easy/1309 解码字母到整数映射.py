'''
给你一个字符串 s，它由数字（'0' - '9'）和 '#' 组成。我们希望按下述规则将s映射为一些小
写英文字符： 字符（'a' - 'i'）分别用（'1' - '9'）表示。字符（'j' - 'z'）分别用
...
提示: 1 <= s.length <= 1000  s[i] 只包含数字（'0'-'9'）和 '#' 字符。
s 是映射始终存在的有效字符串。
'''

def freqAlphabets(s):
    map_dict = dict(zip(range(1, 27), [chr(i) for i in range(97, 123)]))
    n = len(s)
    ans = ''
    i = 0
    while i < n:
        if i < n - 2 and s[i + 2] != '#' or i >= n - 2:
            ans += map_dict[int(s[i])]
            i += 1
        else:
            if i < n - 2:
                ans += map_dict[int(s[i: (i + 2)])]
                i += 3
    return ans
